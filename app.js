...
function weatherLabel(code){code=Number(code);const m={0:'صاف',1:'عمدتاً صاف',2:'نیمه‌ابری',3:'ابری',45:'مه‌آلود',48:'مه یخ‌زن',51:'نم‌نم باران',53:'باران خفیف',55:'باران پیوسته',61:'باران',63:'بارش متوسط',65:'بارش شدید',71:'برف خفیف',73:'برف',75:'برف شدید',80:'رگبار',81:'رگبار متوسط',82:'رگبار شدید',95:'رعدوبرق',96:'رعدوبرق و تگرگ',99:'رعدوبرق شدید'};return m[code]||'وضعیت متغیر'}
function faDate(iso){try{return new Intl.DateTimeFormat('fa-IR',{weekday:'long',month:'long',day:'numeric'}).format(new Date(iso+'T12:00:00'))}catch(e){return iso}}
function faTime(iso){try{return new Intl.DateTimeFormat('fa-IR',{hour:'2-digit',minute:'2-digit',hour12:false}).format(new Date(iso))}catch(e){return iso.slice(11,16)}}
function farmAdvice(c,d){let rain=Number(d?.daily?.precipitation_probability_max?.[0]||0),wind=Number(c?.wind_speed_10m||0),temp=Number(c?.temperature_2m||0);if(rain>=60)return {tone:'info',title:'احتمال بارندگی بالاست',text:'قبل از آبیاری یا کوددهی، بارش امروز را در برنامه کار زمین در نظر بگیر.'};if(wind>=20)return {tone:'warn',title:'برای سم‌پاشی احتیاط کن',text:'سرعت باد نسبتاً بالاست؛ زمان سم‌پاشی را به ساعات کم‌بادتر موکول کن.'};if(temp>=34)return {tone:'warn',title:'گرمای هوا بالاست',text:'آبیاری و فعالیت در زمین را به ساعات خنک‌تر منتقل کن و از تنش گرمایی محصول مراقبت کن.'};if(temp<=3)return {tone:'warn',title:'احتمال سرمای آسیب‌زا',text:'برای محصولات حساس، وضعیت سرما و دمای شب را بررسی کن.'};return {tone:'good',title:'شرایط کلی مناسب است',text:'باد و احتمال بارش فعلاً پایین است؛ برای کارهای معمول مزرعه شرایط نسبتاً مناسبی دیده می‌شود.'}}
async function weatherData(lat,lon,box){try{box.innerHTML='<div class="weather-loading"><div class="spinner"></div><b>در حال دریافت آخرین وضعیت هوا…</b><span>اطلاعات بر اساس موقعیت زمین محاسبه می‌شود.</span></div>';let u=`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,apparent_temperature,relative_humidity_2m,wind_speed_10m,wind_direction_10m,precipitation,weather_code,uv_index&hourly=temperature_2m,apparent_temperature,precipitation_probability,weather_code,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,precipitation_sum,wind_speed_10m_max,uv_index_max,sunrise,sunset&timezone=auto`;let ac=new AbortController(),tm=setTimeout(()=>ac.abort(),12000);let d=await (await fetch(u,{signal:ac.signal})).json();clearTimeout(tm);d=d||{};let c=d.current||{},h=d.hourly||{},days=d.daily||{},advice=farmAdvice(c,d);let hStart=Math.max(0,(h.time||[]).findIndex(x=>x>=c.time));if(hStart<0)hStart=0;let hours=(h.time||[]).slice(hStart,hStart+12);box.innerHTML=`<div class="weather-page-head"><div><span class="eyebrow">وضعیت فعلی</span><h2>${weatherLabel(c.weather_code)}</h2><div class="weather-temp-row"><strong>${Math.round(c.temperature_2m||0)}°</strong><span>احساس ${Math.round(c.apparent_temperature||0)}°</span></div><p class="weather-location">📍 موقعیت انتخاب‌شده · بروزرسانی ${faTime(c.time||new Date().toISOString())}</p></div><div class="weather-big-icon">${weatherIcon(c.weather_code)}</div></div><div class="weather-stats"><div><span>💧 رطوبت</span><b>${Math.round(c.relative_humidity_2m||0)}٪</b></div><div><span>💨 باد</span><b>${Math.round(c.wind_speed_10m||0)} km/h</b></div><div><span>🌧️ بارش</span><b>${Math.round(c.precipitation||0)} mm</b></div><div><span>☀️ UV</span><b>${Math.round(c.uv_index||0)}</b></div></div><div class="farm-advice ${advice.tone}"><div class="advice-icon">${advice.tone==='good'?'✓':advice.tone==='warn'?'!':'i'}</div><div><b>${advice.title}</b><p>${advice.text}</p></div></div><div class="weather-section"><div class="section-head"><h3>پیش‌بینی ساعتی</h3><span class="muted small">۱۲ ساعت آینده</span></div><div class="hourly-row">${hours.map((x,i)=>{let j=h.time.indexOf(x);return `<div class="hour-card ${i===0?'now':''}"><b>${i===0?'اکنون':faTime(x)}</b><span class="hour-icon">${weatherIcon(h.weather_code?.[j])}</span><strong>${Math.round(h.temperature_2m?.[j]||0)}°</strong><small>بارش ${Math.round(h.precipitation_probability?.[j]||0)}٪</small></div>`}).join('')}</div></div><div class="weather-section"><div class="section-head"><h3>پیش‌بینی روزانه</h3><span class="muted small">۷ روز آینده</span></div><div class="forecast-list">${(days.time||[]).slice(0,7).map((x,i)=>`<div class="forecast-card"><div class="forecast-day"><b>${i===0?'امروز':faDate(x)}</b><small>${x}</small></div><div class="forecast-icon">${weatherIcon(days.weather_code?.[i])}</div><div class="forecast-temp"><strong>${Math.round(days.temperature_2m_max?.[i]||0)}°</strong><span>${Math.round(days.temperature_2m_min?.[i]||0)}°</span></div><div class="forecast-rain">🌧️ ${Math.round(days.precipitation_probability_max?.[i]||0)}٪<small>${Number(days.precipitation_sum?.[i]||0).toFixed(1)} mm</small></div></div>`).join('')}</div></div><div class="sun-card"><div><span>🌅 طلوع</span><b>${faTime(days.sunrise?.[0]||'')}</b></div><div><span>🌇 غروب</span><b>${faTime(days.sunset?.[0]||'')}</b></div><div><span>💨 بیشینه باد</span><b>${Math.round(days.wind_speed_10m_max?.[0]||0)} km/h</b></div></div><p class="weather-note">پیش‌بینی‌ها تقریبی‌اند و با تغییر شرایط جوی بروزرسانی می‌شوند. برای تصمیم‌های حساس مزرعه، وضعیت لحظه‌ای را دوباره بررسی کن.</p>`}catch(e){box.innerHTML='<div class="weather-error"><b>دریافت آب‌وهوا انجام نشد.</b><span>اتصال اینترنت یا دسترسی سرویس هواشناسی را بررسی کن.</span><button class="secondary" onclick="renderWeather()">تلاش دوباره</button></div>'}}
function renderWeather(){head('هوا');let lands=state.lands.filter(x=>x.lat&&x.lng),l=lands[0];app.innerHTML=`<div class="weather-shell"><div class="weather-toolbar"><div><span class="eyebrow">هواشناسی مزرعه</span><h2>آب‌وهوای دقیق زمین</h2></div><button class="refresh-weather" id="weatherRefresh">↻ بروزرسانی</button></div><div class="weather-picker card"><label>زمین موردنظر</label><select id="weatherLand"><option value="user">📍 موقعیت من</option>${lands.map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('')}</select></div><div id="weatherMain"></div></div>`;let box=$('#weatherMain');const loadUser=()=>navigator.geolocation?.getCurrentPosition(p=>weatherData(p.coords.latitude,p.coords.longitude,box),()=>{box.innerHTML='<div class="weather-error card"><b>دسترسی به موقعیت فعال نیست</b><span>از تنظیمات گوشی اجازه موقعیت مکانی را برای Chrome فعال کن.</span></div>'},{enableHighAccuracy:true,maximumAge:60000,timeout:12000});if(l)weatherData(l.lat,l.lng,box);else box.innerHTML='<div class="weather-empty card"><div class="empty-icon">☀️</div><h3>آماده دریافت هوا</h3><p class="muted">یک زمین دارای موقعیت را انتخاب کن یا موقعیت فعلی گوشی را بگیر.</p><button class="primary" id="getUserWeather">📍 دریافت آب‌وهوای موقعیت من</button></div>';$('#getUserWeather')?.addEventListener('click',loadUser);$('#weatherLand').onchange=()=>{let v=$('#weatherLand').value;if(v==='user')loadUser();else{let x=land(v);if(x?.lat&&x?.lng)weatherData(x.lat,x.lng,box)}};$('#weatherRefresh').onclick=()=>{let v=$('#weatherLand').value;if(v==='user')loadUser();else{let x=land(v);if(x?.lat&&x?.lng)weatherData(x.lat,x.lng,box)}}}
let map=null,points=[],markers=[],watch=null,layer=null,poly=null,track=false,sat=false,acc=0;
function renderMeasure(){head('متراژ');app.innerHTML=`<div class="measure-page"><div id="measureMap" class="measure-map"></div><div class="measure-overlay"><input id="measureSearch" placeholder="جستجوی روستا، شهر یا مختصات"><button class="primary" id="searchBtn">جستجو</button></div><div class="floating"><button id="locBtn">⌖</button><button id="satBtn">🛰️</button><button id="clearBtn">↺</button><button id="closeMeasure">×</button></div><div class="measure-bottom"><div class="measure-stats"><div><b id="mArea">۰</b><span>مترمربع</span></div><div><b id="mHa">۰</b><span>هکتار</span></div><div><b id="mPer">۰</b><span>متر محیط</span></div></div><div class="accuracy" id="mAcc">آماده پیمایش</div><button class="finish" id="trackBtn">▶ شروع پیمایش GPS</button><div class="measure-help">۳ نقطه روی نقشه بزن یا پیمایش GPS را شروع کن؛ بعد دکمه ثبت فعال می‌شود.</div><button class="primary measure-register" id="useBtn" style="width:100%;margin-top:7px" disabled>📐 ثبت زمین با این مساحت</button></div></div>`;setTimeout(initMap,30)}
function initMap(){if(!window.L){$('#mAcc').textContent='نقشه بارگذاری نشد.';return}map=L.map('measureMap',{zoomControl:false,maxZoom:20,minZoom:3}).setView([35.7,51.4],12);layer=L.layerGroup().addTo(map);baseLayer=L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:20,attribution:'© OpenStreetMap',noWrap:true}).addTo(map);map.on('click',e=>addPoint(e.latlng.lat,e.latlng.lng));points=[];markers=[];updateMeasure()}
